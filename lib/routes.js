...
module.exports.index = function(req, res) {
	...
	if (hook) {
		...
		switch(command) {
			...
			case "open":
				if (params[2] == "doubles") {
					res.json({text: "Only open singles match is supported at this time."})
				} else if (params[2] == "single" || params[2] == "singles") {
					pong.createOpenSinglesChallenge(hook.user_name, function(message) {
						res.json({text: message});
					});
				} else {
					res.json({text: "Only `pongbot open singles` is supported at this time."})
				}
			break;
			case "join":
				pong.join(hook.user_name, function(message) {
					res.json({text: message});
				});
			break;
			case "accept":
			...
			case "trash":
				var gifs = [
					"http://rs1img.memecdn.com/1800s-trash-talk_o_1280543.jpg",
					"https://kelseykaypernick.files.wordpress.com/2014/01/blog88.jpg",
					"http://rs1img.memecdn.com/ash-talks-trash_o_420449.jpg",
					"http://www.quickmeme.com/img/d8/d8c2188c371c0aef5987d571d31bc9601ab8ccdb2eab583833623422fde0223b.jpg"
				]
				var rand = gifs[Math.floor(Math.random() * gifs.length)];
				res.json({text: rand});
			break;
			case "apologise":
				var message="http://cdn.meme.am/instances/500x/54628491.jpg";
				res.json({text: message});
			break;
			default:
